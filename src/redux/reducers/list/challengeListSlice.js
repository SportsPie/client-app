...
const initialState = {
  ...
  loading: true,
  isLast: false,
  pagingKey: Math.floor(Math.random() * 10000),
};
...
export const challengeListSlice = createSlice({
  ...
  reducers: {
    ...
    setPagingKey: (state, actions) => {
      state.pagingKey = actions.payload;
    },
    ...
    refresh: (state, actions) => {
      ...
      state.isLast = false;
      state.list = [];
      state.pagingKey = Math.floor(Math.random() * 10000);
      ...
      if (state.list && state.list.length > 0) {
        ...
        state.list = state.list.map(v => {
          if (Number(v[idxName]) === Number(idx)) {
            const obj = { ...v, ...item, cntVideo: v.cntVideo };
            obj[idxName] =
...
export const challengeListAction = challengeListSlice.actions;
