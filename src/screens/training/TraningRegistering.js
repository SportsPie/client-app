...
import { apiSaveMasterVideo } from '../../api/RestAPI';
...
function TraningRegistering({ route }) {
  ...
  const saveMasterVideo = async () => {
    try {
      ...
      if (data) {
        ...
        NavigationService.replace(navName.traningClear, {
          trainingIdx,
          videoIdx,
        });
      }
...
export default memo(TraningRegistering);
