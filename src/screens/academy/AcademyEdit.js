...
export default function AcademyEdit({ route }) {
  ...
  // 삭제 plan
  const [removePlanTypeList, setRemovePlanTypeList] = useState([]); // idxs
  ...
  const getAcademyDetailInfo = async () => {
    try {
      ...
      setAcademyName(data.data.academy.academyName);
      const geoResult = await GeoLocationUtils.getLngLat(
        data.data.academy?.addressFull,
      );
      const jibunAddress = geoResult?.jibunAddress;
      let detailAddrStr = data.data.academy?.addressFull;
      if (jibunAddress) {
        detailAddrStr = detailAddrStr.replace(jibunAddress, '').trim();
        setAddr(jibunAddress);
      }
      setDetailAddr(detailAddrStr);
      setCeoPhone(data.data.academy?.phoneNo);
      setWorkTime(data.data.academy?.workTime);
      setInstagram(data.data.academy?.instagramUrl);
      setHomePageUrl(data.data.academy?.homepageUrl);
      setDescription(data.data.academy?.description);
      setLatitude(data.data.academy?.latitude);
      setLongitude(data.data.academy?.longitude);
      setPrevPhotoList(data.data.academy?.files || []);
      if (
        data.data.academy?.planTypeList &&
        data.data.academy?.planTypeList.length > 0
      ) {
        data.data.academy?.planTypeList.forEach(item => {
          if (item.planTypeCode.includes('CLS_')) {
            setSelectedClassType(prev => {
              return { ...prev, [item.planTypeCode]: !!item.planTypeCode };
            });
            if (item.planTypeCode.includes('ETC')) setClassDesc(item.etc);
          }
          if (item.planTypeCode.includes('MTD_')) {
            setSelectedTeachingType(prev => {
              return { ...prev, [item.planTypeCode]: !!item.planTypeCode };
            });
            if (item.planTypeCode.includes('ETC')) setTeachingDesc(item.etc);
          }
        ...
        setRemovePlanTypeList(
          data.data.academy.planTypeList.map(item => item.planIdx),
        );
      }
      setMemo(data.data.academy?.memo);
      setAutoJoin(data.data.academy?.autoApprovalYn);
      setMatchHistoryOpen(data.data.academy?.openMatchPublicYn);
    } catch (error) {
      handleError(error);
    }
    setFstCall(true);
  ...
  const modifyAcademy = async () => {
    try {
      ...
      const params = {
        ...
        removePlanTypeList:
          removePlanTypeList && removePlanTypeList.length > 0
            ? removePlanTypeList
            : null, // idxs
        removeFiles: removeFiles && removeFiles.length > 0 ? removeFiles : null, // idxs
      };
      formData.append('dto', {
      ...
      setTimeout(() => {
        Utils.openModal({
          title: '성공',
          body: '아카데미 수정이 완료되었습니다.',
          closeEvent: MODAL_CLOSE_EVENT.movePage,
          pageName: navName.academyDetail,
          data: { academyIdx },
        });
      }, 0);
    } catch (error) {
  ...
  const updateAcademyPhoto = ({ fileUrl, imageName, imageType }) => {
    ...
    if (prevPhotoList.length + academyPhotoList.length > 2) return; // 이미지는 3개까지 업로드 가능
    setAcademyPhotoList(prev => [...prev, photo]);
  };

  const removePrevAcademyPhoto = (index, idx) => {
  ...
  const checkValues = () => {
    ...
    if (!((academyPhotoList?.length || 0) + (prevPhotoList?.length || 0) > 0)) {
            ...
            <View style={styles.container}>
              <ScrollView
                  ...
                  <TouchableOpacity
                    ...
                    <View
                      ...
                      academyDetail.logoPath ? (
                        <Image
                          source={{
                            uri: logoImage?.uri || academyDetail.logoPath,
                          }}
                          style={{
                            width: 56,
                            height: 56,
                ...
                <View style={styles.contentsList}>
                  ...
                  <View>
                    ...
                    <View
                          ...
                          return (
                            <View
                              ...
                              <TouchableOpacity
                                onPress={e => {
                                  e.stopPropagation();
                                  removePrevAcademyPhoto(index, item.fileIdx);
                                }}
                                style={{
                                  position: 'absolute',
                                  right: 4,
                      ...
                      {prevPhotoList.length + academyPhotoList.length < 3 && (
                        <TouchableOpacity
                          onPress={() => {
                ...
                <View style={[styles.contentsList, { paddingTop: 24 }]}>
                  ...
                  <View>
                    <Text style={styles.subTitle}>인스타그램</Text>
                    <TextInput
                      value={instagram}
                      onChange={e => {
