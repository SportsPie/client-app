...
function EditVideoDetails({ route }) {
  ...
  // 전달 파라미터 > 접근 유효성 검증
  const { eventIdx, video, uploadType } = route?.params || {
    eventIdx: '',
    video: null,
    uploadType: '',
  };
  ...
  if (uploadType === VIDEO_UPLOAD_TYPE.EVENT && !eventIdx) {
    handleError(new AccessDeniedException('잘못된 접근입니다.'));
  }
  ...
  const modifyVideo = async () => {
    try {
      ...
      // API 호출
      if (uploadType === VIDEO_UPLOAD_TYPE.EVENT) {
        const params = {
          eventIdx,
        ...
        const { data } = await apiPutEventVideo(params);
  ...
  useEffect(() => {
    ...
    setDescription(video?.contents);
    setIsOpenVideo(video?.showYn === 'Y');
    setIsRepresentative(video?.fixDate != null);
  }, []);
...
export default memo(EditVideoDetails);
