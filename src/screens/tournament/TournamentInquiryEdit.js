...
function TournamentInquiryEdit({ route }) {
  /**
   * states
   */
  ...
  const [removedFiles, setRemovedFiles] = useState([]); // idxs
  ...
  const getInquiryData = async () => {
    try {
      ...
      setInquiryData(data.data?.qna);
      setTitle(data.data?.qna?.title || '');
      setContent(data.data?.qna?.question || '');
  ...
  const regist = async () => {
    ...
    try {
      ...
      const { data } = await apiPostTournamentQnaInsert(formData);
      dispatch(tournamentInquiryListAction.refresh());
      Utils.openModal({
        title: '성공',
        body: '문의가 저장되었습니다.',
  ...
  const modify = async () => {
    ...
    try {
      ...
      const { data } = await apiPutTournamentQnaModify(formData);
      // dispatch(tournamentInquiryListAction.refresh());
      Utils.openModal({
        title: '성공',
        body: '문의가 수정되었습니다.',
...
export default memo(TournamentInquiryEdit);
